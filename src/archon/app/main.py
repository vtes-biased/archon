...
async def sync_vekn_members(op: db.Operator) -> None:
    ...
    async for members in vekn.get_members_batches():
        ...
        try:
            ...
            LOG.exception("Failed to insert members batch")
        del members
    for prefix, uid in prefixes_map.items():
        try:
            ...
            LOG.exception("Failed to set sponsor for prefix %s", prefix)
    del prefixes_map
    ...
    async with db.operator(autocommit=True) as op:
        ...
        async for event in vekn.get_events_serial(members):
            try:
                ...
                LOG.exception(
                    "Failed to upsert tournament vekn_id=%s",
                    event.extra.get("vekn_id"),
                )
            del event
        del members
...
async def push_to_vekn() -> None:
    """Push Archon-created members and tournaments to vekn.net.

    Call this at bootstrap to sync data that was created while VEKN_PUSH was disabled.
    Members are synced first (they may be tournament participants).
    """
    async with db.operator(autocommit=True) as op:
        # First: push members created by Archon (VEKN >= 1000000, has sponsor)
        async with op.conn.cursor() as cursor:
            res = await cursor.execute(
                "SELECT data FROM members "
                "WHERE vekn >= '1000000' "
                "AND (data->>'sponsor') IS NOT NULL "
                "AND (data->>'sponsor') <> ''"
            )
            members_to_push = [row[0] for row in await res.fetchall()]

        LOG.info("Found %d members to push to VEKN", len(members_to_push))
        for member_data in members_to_push:
            try:
                member = db.Operator._instanciate(op, member_data, db.models.Member)
                await vekn.create_member(member)
                LOG.info("Pushed member %s (VEKN: %s)", member.name, member.vekn)
            except Exception:
                LOG.exception(
                    "Failed to push member %s (VEKN: %s)",
                    member_data.get("name"),
                    member_data.get("vekn"),
                )
        ...
        async with op.conn.cursor() as cursor:
            res = await cursor.execute(
                ...
                [db.models.TournamentState.FINISHED],
            )
            tournaments_to_push = [row[0] for row in await res.fetchall()]

        LOG.info("Found %d tournaments to push to VEKN", len(tournaments_to_push))
        for t_data in tournaments_to_push:
            try:
                tournament = db.Operator._instanciate(
                    op, t_data, db.models.Tournament
                )
                # Need organizer VEKN to create event
                if not tournament.judges:
                    LOG.warning(
                        "Tournament %s has no judges, skipping", tournament.name
                    )
                    continue
                organizer_vekn = tournament.judges[0].vekn
                if not organizer_vekn:
                    LOG.warning(
                        "Tournament %s organizer has no VEKN, skipping", tournament.name
                    )
                    continue
                ...
                if players_without_vekn:
                    LOG.warning(
                        "Tournament %s has players without VEKN: %s, skipping",
                        tournament.name,
                        players_without_vekn,
                    )
                    continue
